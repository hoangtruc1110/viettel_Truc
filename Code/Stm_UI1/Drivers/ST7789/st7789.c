...
//uint8_t disp_buf[ST7789_HEIGHT * HOR_LEN*2];
uint8_t disp_buf[ST7789_HEIGHT * ST7789_WIDTH];
#endif
...
static void ST7789_WriteCommand(uint8_t cmd)
{
	ST7789_Select();
	ST7789_DC_Clr();
	if(HAL_SPI_Transmit(&ST7789_SPI_PORT, &cmd, sizeof(cmd), HAL_MAX_DELAY) != HAL_OK){
		Error_Handler();
	}
	ST7789_UnSelect();
}
...
void ST7789_Init(void)
{
	...
	HAL_Delay(50);
//	ST7789_SetAddressWindow(0, 0, 0, 0);
...
//		return;
	...
	ConvHL(data, (int32_t)(sizeof(uint16_t) * w * h));
//	ST7789_WriteData(data, sizeof(uint16_t) * w * h);
	ST7789_Select();
	ST7789_DC_Set();
	HAL_SPI_Transmit_DMA(&ST7789_SPI_PORT, data, sizeof(uint16_t) * w * h);
	while (ST7789_SPI_PORT.hdmatx->State != HAL_DMA_STATE_READY)
					{}
	ST7789_UnSelect();
}
